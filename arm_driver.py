...
# 线性 (仿射) 变换, 6 组标定点最小二乘拟合 (手动标定, 最大残差 ~3.4 px ≈ 1.3 mm;
# 另 2 组点 臂(150,-99)/(103,-116) 测量误差大已剔除):
#   臂(100,  -10) → 相机(136,  80)   臂(265,  -8) → 相机(560,  80)
#   臂(145,  -24) → 相机(254, 121)   臂(192, -66) → 相机(373, 227)
#   臂(244,  -64) → 相机(503, 218)   臂(266, -117) → 相机(558, 350)
#   u = 2.557284675978·x + 0.035100527897·y - 117.5474957872
#   v = 0.0111350590443·x - 2.490358102413·y + 57.13180280682
#  CAMERA_A[0] = u 的 (x, y) 系数, CAMERA_A[1] = v 的 (x, y) 系数, CAMERA_A[2] = 常数项
CAMERA_A = (
    ( 2.557284675978e+00,  3.510052789690e-02),
    ( 1.113505904431e-02, -2.490358102413e+00),
    (-1.175474957872e+02,  5.713180280682e+01),
)
