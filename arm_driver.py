...
# 线性 (仿射) 变换, 4 组标定点最小二乘拟合 (手动标定, 最大残差 ~0.6 px ≈ 0.2 mm):
#   臂( 97.4,  -1.3) → 相机(130,  78)   臂(264.0,   7.4) → 相机(552,  82)
#   臂(106.5, -113.2) → 相机(136, 355)   臂(269.4, -101.4) → 相机(547, 351)
#   u = 2.518140884989·x + 0.160762408306·y - 114.5265780927
#   v = 0.153283761920·x - 2.463861816593·y + 59.81717339422
#  CAMERA_A[0] = u 的 (x, y) 系数, CAMERA_A[1] = v 的 (x, y) 系数, CAMERA_A[2] = 常数项
CAMERA_A = (
    ( 2.518140884989e+00,  1.607624083056e-01),
    ( 1.532837619199e-01, -2.463861816593e+00),
    (-1.145265780927e+02,  5.981717339422e+01),
)
